...
public class AverageIfs : FunctionBase
{
    ...
    public AverageIfs(params string[] arguments)
    {
    ...
    public override JLioFunctionResult Execute(JToken currentToken, JToken dataContext, IExecutionContext context)
    {
        ...
        if (values.Count < 3 || values.Count % 2 == 0)
        {
            context.LogError(CoreConstants.FunctionExecution,
                $"{FunctionName} requires an odd number of arguments: average_range, criteria_range1, criteria1, [criteria_range2, criteria2], ...");
            return JLioFunctionResult.Failed(currentToken);
        }

        var averageRangeValue = values[0];
        ...
        var averageRange = ExtractArray(averageRangeValue.Value);
        ...
        for (int i = 0; i < averageRange.Count; i++)
        {
            ...
            if (allCriteriaMatch)
            {
                if (TryGetNumericValue(averageRange[i], out var numericValue))
                {
                    sum += numericValue;
                    count++;
                }
