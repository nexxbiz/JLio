...
public class Contains : FunctionBase
{
    public Contains()
    {
    }

    public Contains(params string[] arguments)
    {
    ...
    public override JLioFunctionResult Execute(JToken currentToken, JToken dataContext, IExecutionContext context)
    {
        ...
        if (values.Count < 2 || values.Count > 3)
        {
            context.LogError(CoreConstants.FunctionExecution,
                $"{FunctionName} requires 2 or 3 arguments (text, searchValue, [ignoreCase])");
        ...
        var result = text.Contains(searchValue, comparison);
