    ...
    public override JLioFunctionResult Execute(JToken currentToken, JToken dataContext, IExecutionContext context)
    {
        ...
        if (values.Count < 3 || values.Count % 2 == 0)
        {
            context.LogError(CoreConstants.FunctionExecution,
                $"{FunctionName} requires an odd number of arguments: max_range, criteria_range1, criteria1, [criteria_range2, criteria2], ...");
            return JLioFunctionResult.Failed(currentToken);
        }

        var maxRangeValue = values[0];
        if (!maxRangeValue.WasFound)
        {
            context.LogError(CoreConstants.FunctionExecution,
                $"{FunctionName} max_range argument path not found");
            return JLioFunctionResult.Failed(currentToken);
        }

        var maxRange = ExtractArray(maxRangeValue.Value);
        ...
        for (int i = 0; i < maxRange.Count; i++)
        {
            ...
            if (allCriteriaMatch)
            {
                if (TryGetNumericValue(maxRange[i], out var numericValue))
                {
                    if (!hasValue || numericValue > maxValue)
                    {
